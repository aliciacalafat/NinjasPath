...
public class MisionManager : Singleton<MisionManager>
{
    ...
    public void AñadirProgreso(string misionID, int cantidad)
    {
        ...
        if (misionPorActualizar.MisionAceptada)
        {
    ...
    //Ciclo for que recorra nuestra array de mision disponibles y que
    //para cada uno de ellos llame a su método resetquest.
    private void OnEnable()
    {
        for (int i = 0; i < misionesDisponibles.Length; i++)
        {
            misionesDisponibles[i].ResetMision();
        }
